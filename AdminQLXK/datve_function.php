...
function quandi_filter($matinhdi)
{
    ...
    echo '<select class="form-select1 text-dark text-center" style="border: none" id="quanhuyendi" name="quanhuyendi" onchange="getDivBenDi()">';
    echo '<option selected text-dark>Chọn quận huyện đi</option>';
...
function quanden_filter($matinhden)
{
    ...
    $result = mysqli_query($conn, $sql);
    echo '<select class="form-select1 text-dark text-center" style="border: none" id="quanhuyenden" name="quanhuyenden" onchange="getDivBenDen()">';
    echo '<option selected text-dark>Chọn quận huyện đến</option>';
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            echo '<option value="' . $row["MAQUANHUYEN"] . '">' . $row["TENQUANHUYEN"] . '</option>';
        }
...
function bendi_filter($matinhdi, $maquanhuyen)
{
    ...
    $result = mysqli_query($conn, $sql);
    echo '<select class="form-select1 text-dark text-center" style="border: none" id="bendi" name="bendi">';
    echo '<option selected text-dark>Chọn bến đi</option>';
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
...
function benden_filter($matinhden, $maquanhuyen)
{
    ...
    $result = mysqli_query($conn, $sql);
    echo '<select class="form-select1 text-dark text-center" style="border: none" id="benden" name="benden">';
    echo '<option selected text-dark>Chọn bến đến</option>';
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            echo '<option value="' . $row["MABX"] . '">' . $row["TENBEN"] . '</option>';
        }
...
function get_TableGhe($biensoxe, $idchuyenxe)
{
    ...
    echo '</div>';
    echo '<div class="row">';
    echo '<button class="btn btn-info" style="width: 70%; " id="bookticket"  type="button" >Đặt vé</button>';
    echo '</div>';
    echo '</div>';
    echo '</div>';
    echo '</div>';
...
//LOAD XÁC NHẬN THÔNG TIN ĐẶT VÉ

if (isset($_GET["function"]) && $_GET["function"] == "xacnhanthongtin") {
    $hoten = $_GET["hoten"];
    $email = $_GET["email"];
    $sdt = $_GET["sdt"];
    $_SESSION["email"] = $email;
    echo xacnhanthongtin($hoten, $email, $sdt);
}

function xacnhanthongtin($hoten, $email, $sdt)
{
    $conn = connect();
    ...
    echo '<div class="col m-3 p-5 border-right">';
    echo '<div class="row border mt-3" style="border-radius:10px; background-color: #f7f7f7;">';
    echo '<div class="m-4">';
    echo '<p><b style="color: #ba2f25;">Họ tên hành khách: </b>' . $hoten . '</p>';
    echo '<p><b style="color: #ba2f25;">Số điện thoại: </b>' . $sdt . '</p>';
    echo '<p><b style="color: #ba2f25;">Email: </b>' . $email . '</p>';
    if ($_SESSION["khuhoi"] == 1) {
